...
const ToNext: React.FC<{ active: string }> = ({ active }) => {
	return (
		<button
			title="Next section"
			type="button"
			className="mobileHover:dark:hover:bg-blue-600 mobileHover:hover:bg-amber-400 rounded-full h-12 w-12 flex items-center justify-center transition-colors group -translate-y-1 cursor-pointer"
			onClick={() => {
